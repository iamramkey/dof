...
/*
	this is the vpn bulk table name
*/
$vpnBulkTableName = 'vpn_bulk';
...
/*
	this is the vulnerability ip bulk table name
*/
$vulBulkTableName = 'vulnerabilty_bulk';
...
if(count($result) > 0){
	foreach($result as $x) {
		...
		if(!empty($res)){
			...
			$insertSQL = "INSERT INTO `$vpnBulkTableName` SET ";
...
$vpnBulkFile = 'XMLFiles/VPN_Bulk.xml';

$elog->write("Writing to Bulk XML file");
$result = $db1->exec("select * from $vpnBulkTableName");
$elog->write("$vpnBulkTableName is having " . count($result) . " rows");
$insertArr = [];
if(count($result) > 0){
...
$single->endElement();
if(file_put_contents($vpnBulkFile,$single->outputMemory()) !== false){
...
if(count($result) > 0){
	foreach($result as $x) {
		$finalResult = $x;
		$insertSQL = "INSERT INTO `vulnerabilty_bulk` SET ";
		foreach ($finalResult as $field => $value) {
			if($field == 'id'){
				continue;						
			}
...
$vulBulkFile = 'XMLFiles/Vulnerabilty_Bulk.xml';

$elog->write("Writing to Bulk XML file");
$result = $db1->exec("select * from $vulBulkTableName");
$elog->write("$vulBulkTableName is having " . count($result) . " rows");
$insertArr = [];
if(count($result) > 0){
	$single = new XMLWriter();
	$single->openMemory();
...
$single->endElement();
if(file_put_contents($vulBulkFile,$single->outputMemory()) !== false){
	$elog->write("Successfully written all records to bulk xml file");
}else{
	$elog->write("there was an error writing data to bulk xml file");
}	
